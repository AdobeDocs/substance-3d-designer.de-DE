---
helpx_url: "https://helpx.adobe.com/de/substance-3d-designer/scripting.html"
breadcrumb-title: ''
description: Erfahren Sie, wie Sie die Substance 3D Designer-Funktionalität mithilfe von Python-Skripten und der Plug-in-Entwicklung erweitern.
helpx_creative_field: ""
helpx_description: Designer > Scripting
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Skripting
user-guide-description: ''
user-guide-title: ''
source-git-commit: 6c55ac0f1f6da5bc5683a34a4eca174f978eac64
workflow-type: tm+mt
source-wordcount: '154'
ht-degree: 4%

---


# Skripting

Es ist möglich, die Anwendung durch das Schreiben von Plugins zu steuern. Plug-Ins können in der Skriptsprache **Python** mit der Version **3.9** geschrieben werden. Die in der Anwendung bereitgestellte Python-API ermöglicht den Zugriff auf viele Funktionen und deren Verwendung direkt aus Skripten.

[Python](https://www.python.org/) ist eine Skriptsprache, die in vielen Branchen weit verbreitet ist. Es hilft u. a. bei der Erstellung von Automatisierungs-, Workflow-Beschleunigungs- und Asset-Management-Tools. Auf die API-Dokumentation kann über das Hauptmenü mit **Hilfe** > **Skriptdokumentation zugegriffen werden...** Aktion.

## Inhaltsverzeichnis

* [Grundlagen zu Plug-ins](../scripting/plugin-basics/plugin-basics.md)
* [Suchpfade einfügen](../scripting/plugin-search-paths/plugin-search-paths.md)
* [Plug-ins > Pakete](../scripting/plugins-packages/plugins-packages.md)
* [Plugin-Manager](../scripting/plugin-manager/plugin-manager.md)
* [Python-Editor](../scripting/python-editor/python-editor.md)
* [Zugreifen auf Diagramme und Auswahlen](../scripting/accessing-graphs-and-sel/accessing-graphs-and-selections.md)
* [Knoten und Eigenschaften](../scripting/nodes-and-properties/nodes-and-properties.md)
* [Rückgängig machen und Wiederholen](../scripting/undo-and-redo/undo-and-redo.md)
* [Anwendungsrückrufe](../scripting/application-callbacks/application-callbacks.md)
* [Erstellen von Benutzeroberflächenelementen](../scripting/creating-user-interface/creating-user-interface-elements.md)
* [Hinzufügen von Aktionen zur Explorer-Symbolleiste](../scripting/adding-actions-the-exp/adding-actions-to-the-explorer-toolbar.md)
* [Farbmanagement verwenden](../scripting/using-color-management/using-color-management.md)
* [Verwenden von Volltonfarben](../scripting/using-spot-colors/using-spot-colors.md)
* [Protokollieren](../scripting/logging/logging.md)
* [Verwenden von Threads](../scripting/using-threads/using-threads.md)
* [Debuggen von Plug-Ins mit Visual Studio Code](../scripting/debugging-plugins-using/debugging-plugins-using-visual-studio-code.md)
* [Portieren früherer Plug-ins](../scripting/porting-previous-plugins/porting-previous-plugins.md)
* [Plug-ins verpacken](../scripting/packaging-plugins/packaging-plugins.md)
* [Scripting-API-Referenz](../scripting/scripting-api-reference/scripting-api-reference.md)
