---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/texture-generators/noises/grunge-scratches-fine.html"
breadcrumb-title: ""
description: Verwenden Sie den Knoten Schmutz Scratches Fein , um feine Kratzmuster für subtilen Verschleiß und Oberflächendetaileffekte hinzuzufügen.
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Texture Generators > Noises > Grunge Scratches Fine
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Schmutz Scratches Bußgeld
user-guide-description: ""
user-guide-title: ""
source-git-commit: 0f214099ae94088d37122a5d474d3e70d4ccf46f
workflow-type: tm+mt
source-wordcount: '138'
ht-degree: 2%
---

# Schmutz Scratches Bußgeld

<table>
<tr style="border: 0;">
<td width="33.33%" style="border: 0;" valign="top">

![](grunge-scratches-fine.resources/grungescratchesfine.jpg){width="200px"}

<b>In:</b> Textur Generators > Rauschen

</td>
<td width="100.00%" style="border: 0;" valign="top">

## Beschreibung

Der Knoten **Schmutz Scratches Fine** generiert eine Schmutz-Map, die einer fein zerkratzten sauberen Oberfläche ähnelt.

</td>
</tr>
</table>

<a name="parameters"></a>

## Parameter

|  |  |
|:---|:---|
| <b>Saldo</b> <i>Gleitend</i> | Passt die Balance zwischen dunklen und hellen Werten an. |
| <b>Kontrast</b> <i>Gleitend</i> | Passt den Kontrast des Bildes an. |
| <b>Umkehren</b> <i>Boolescher Wert</i> | Kehrt die Ausgabe des Bildes um, indem ein `1-x`-Vorgang verwendet wird. |
| <b>Quadratische Ausbreitung</b> <i>Boolescher Wert</i> | Ermöglicht die Kompensation von Quetsch und Dehnung bei nicht quadratischen Verhältnissen. |
| <b>Erweitert</b> |  |
| <b>Anzahl Scratches</b> <i>Gleitend</i> | Passt die Anzahl feiner Kratzer auf der Oberfläche an. |
| <b>Intensität schärfen</b> <i>Gleitend</i> | Passt die Intensität des globalen Scharfzeichnungseffekts an. |
| <b>Bias für Arbeitsvolumes</b> <i>Fließkommazahl</i> | Passt die Balance der Luminanzen an, die einzelnen Kratzern zugeordnet sind. |

## Beispiele

<table style="table-layout:fixed">
    <tr style="border: 0;">
        <td style="border: 0;">
            <img src="grunge-scratches-fine.resources/grungescratchesfine-variant.jpg" class="modal-image" alt="Geldbuße für Schmutz-Scratches - Beispiel 1" />
        </td>
        <td style="border: 0;">
            <img src="grunge-scratches-fine.resources/grungescratchesfine-variant2.jpg" class="modal-image" alt="Geldbuße für Schmutz-Scratches - Beispiel 2" />
        </td>
        <td style="border: 0;"></td>
    </tr>
</table>
