---
helpx_url: "https://helpx.adobe.com/de/substance-3d-designer/release-notes/old-versions/version-2020-2-10-2.html"
breadcrumb-title: ''
...
* **Verbesserte Menüaktionen** Aktionen zum leg und Bearbeiten von freigelegten Parametern wurden getrennt, um expliziter zu sein. Neue Tastaturbefehle wurden auch eingeführt, um freigelegte Parameter einfacher und schneller bearbeiten zu können. So gibt es z.B. jetzt eine eigene Schaltfläche, um die Funktion eines Grafen zu bearbeiten, und eine eigene Aktion, um zum gelegt Parameterparameter zu springen.

  ![](../../../assets/param-edit.png)

  ![](../../../assets/param-expose.png)

* **Neuen Parameter direkt aus dem Dialogfeld konfigurieren** Wenn ein Parameter gelegt wird, können alle regulären Steuerelemente, wie die Beschreibung und die Standardwerte, jetzt über das Dialogfeld **Parameter Leg** aufgerufen werden. Dadurch wird das Konfigurieren und Erstellen neuer Parameterparameter wesentlich einfacher und schneller, sodass Sie nicht sofort nach dem Erstellen eines Grafen in die Parameteransicht des Grafen wechseln müssen.

  ![](../../../assets/expose-param-window.png){width="600px"}

### Verbesserte Symbolkonfiguration für Substance Graf

![](../../../assets/banner-thumbnail-generator.jpg)

Das Einrichten von Miniaturansichten im Substance-Graf ist mit der verbesserten Benutzeroberfläche jetzt viel einfacher:

* **Automatische Generierung von Graf-Miniaturansichten**\
  Es ist jetzt möglich, automatisch eine Vorschau für einen Substance-Graf zu generieren, indem Sie einfach auf die Schaltfläche Generieren neben dem Symbol in den Graf-Parametern klicken. Es unterstützt derzeit nur Graf, der den PBR-Workflow für Metallic/Rauheit als Ausgabeknoten verwendet. Bei der Erzeugung der Miniaturansicht wird die Intensität des Versatzes aus dem Wert der Physische Größe berechnet, falls definiert, andernfalls wird standardmäßig der Wert 0,1 verwendet.
...
In dieser neuen Version wurden zwei neue Optimierungen vorgenommen, die die Zeit zwischen den Iterationen verkürzen:

* **Verbesserte Leistung der Graf-Berechnung**\
  Ausgabeknoten werden jetzt direkt berechnet, wenn sie angefordert werden, anstatt die Zwischenknoten auf dem Weg zu berechnen (was jetzt in einem zweiten Schritt ist). Diese Änderung ermöglicht es, die Ergebnisse eines Grafen viel schneller in der Vorschau anzuzeigen, selbst wenn Stammknoten angepasst werden. Im folgenden Vergleich ermöglicht diese Optimierung, mehr Iterationen des Grafen im selben Zeitraum zu sehen (hier mit einem Material mit einer Auflösung von 4K):

  ![](../../../assets/perf-graph.gif)

* **Verbesserte Ausdehnung der Baker und Generierung der Diffusion**\
  Die Generierung der Ausdehnung und Diffusion Baking geführt Texturen ist jetzt bis zu viermal schneller als in früheren Versionen. Dadurch wird die Wartezeit zwischen den Baking führte reduziert.
...
* [Iray] Sichtbare Flächen auf nicht-opaken Flächen
* [Vorgaben] Absturz beim Anwenden von Vorgaben in Instanzen einiger Substance Source-Graf
