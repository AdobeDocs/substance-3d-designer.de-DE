...
* **Volumenstreuung**\
  ...
  * **Streuung**: Steuert die Lichtintensität innerhalb des Materials.
  ...
  Hinweis: Subsurface benötigt den neuen Shader &quot;physically\_metallic\_roughness\_sss&quot; . Die Farbstreuung kann in den Shader-Parametern geregelt werden.

  ![](version-2019-2-9-2.resources/sub-surface-scattering-0.jpg){width="400px"}

>[!NOTE]
>
> All diese neuen Rendering-Funktionen sind dank einer Überarbeitung des Shader-Systems von Substance Designer möglich. Das **GLSLFX**-Format unterstützt jetzt die Definition von &quot;**Techniques**&quot;, die einen mehrfachen Rendervorgang zulässt. Einige Beispiele finden Sie in unseren Standard-Shadern unter: **Substance Designer/resources/view3d/shaders**

### Verbesserte Leistung

![](version-2019-2-9-2.resources/library-loading-optim.gif)

Es wurden einige Verbesserungen am Substance-Engine (das zu Version 7.2.0 wechselt) vorgenommen, die in mehreren Situationen von Vorteil sind:

* **Zwischenspeichern von Graf-Berechnungen zum schnelleren Anpassen**\
  Das Graf-Rendering wurde durch Hinzufügen eines neuen Caching-Systems verbessert. Dies macht das Anpassen eines Grafen viel schneller als zuvor, da wir jetzt unveränderte Knoten überspringen.
...
Mit dieser neuen Version stellen wir eine Menge neuer Filter vor, die der Manipulation von HDR Umgebungs-Map gewidmet sind.

* **HDR. Führen Sie**:Bring mehrere LDR-Fotos (Low Dynamic Range) zusammen, um ein HDR zu erstellen.
* Der **Nadir Patch**:This-Knoten ermöglicht das Klonen/Ausbessern des Boden-Teils eines HDRI, wobei die sphärische Deformation berücksichtigt wird.
* **Nadir Extract**: Dieser Node kann eine Textur aus einem HDRI extrahieren. Sie kann verwendet werden, um eine Boden-Textur in einer 3D-Szene zuzuordnen.
* **Color Temperature Adjustment**: Passen Sie die Farbtemperatur eines Eingabebilds an, einschließlich der Unterstützung von HDR.
* **Horizont begradigen**: Drehen Sie den Breiten-/Längengrad einer Umgebungs-Map. Je nach Aufnahmebedingungen ist das resultierende 360°-Panorama möglicherweise nicht gerade.
...
* **Flachlicht**: Generieren Sie ein rechteckiges Licht, als wäre es eine 3D-Form. Sie können ihre Position in der 2D-Ansicht ändern, eine Textur anwenden und die Farbe mit einem Temperatur- oder RGB-Wert einstellen.
...
* [3D-Ansicht] Support-Volumenstreuung
* [Graf] Punktknoten
* [Graf] Optimieren des Graf-Renderings durch Zwischenspeichern von Kochergebnissen
