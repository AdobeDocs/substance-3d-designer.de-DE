...
* **Verbesserte Leistung auf Bildschirmen mit hoher DPI-Auflösung mit Verkleinerung des Ansichtsports**\
  Ähnlich wie &quot;Substance Painter&quot; gibt es jetzt einen neuen Parameter namens **Bildschirmskalierung** in den Hauptvoreinstellungen, der erkennt, dass ein Viewport HDPI-Skalierung verwendet (z. B. Retina-Bildschirme auf MacOS) und die Bildschirmauflösung automatisch durch 2 dividiert. Durch dieses Verhalten wird vermieden, dass der Viewport mit zu hohen Auflösungen gezeichnet wird, und die allgemeine Leistung wird verbessert.\
  Die aktuellen Einstellungswerte sind:

  * **Keine**: Kein Skalieren des Viewports.
  * **Auto** (Standard): Wenn sich die Anwendung auf einem HDPI-Bildschirm befindet, wird die Auflösung des Viewports durch zwei dividiert.
...
* **Verbesserter PBR-Rendering-Knoten**\
  Dieser Knoten wurde bereits in einer früheren Version eingeführt und erheblich verbessert. Es bietet jetzt Steuerelemente für die Kamera, neue Formen (z. B. eine Ebene und einen Zylinder), Nacheffekte (Tiefe von Halbbild, Blüte/Blendeffekt, Vignette usw.) und neue Material-Typen (z. B. Klarlack und Anisotropie) sowie Unterstützung für Materialien mit Deckkraft.\
...
* **Neuer FXAA-Filter**\
  Der neue Filter implementiert den Fast Approximate Anti-Aliasing (FXAA)-Algorithmus, bei dem es sich um eine Anti-Aliasing-Methode handelt. Sie können sie in gleichmäßiger Textur verwenden, um gezackte Linien oder Kanten zu glätten.\
