...
**Substance Designer 2020.1** enthält einen neuen Tastaturbefehl-Editor, ein MaterialX-Plug-in und viele neue Verbesserungen für das Farbmanagement mit Adobe Color Engine- und neuen Bäckerfunktionen.
...
### Neuer Verknüpfungs-Editor für die Knotenerstellung

![](../../../assets/banner-shortcuts.png)

In dieser Version wird ein neuer Editor für Tastaturbefehle eingeführt, mit dem Sie Tastaturbefehle definieren können, um die Erstellung von Knoten in den verschiedenen Grafikmodi zu beschleunigen.

Der neue Editor für Tastaturbefehle ist in den Hauptvoreinstellungen über **Bearbeiten > Voreinstellungen > Tastaturbefehle** zugänglich. Standardmäßig sind alle Verknüpfungsfelder leer.

* **Kurzbefehle für jeden Diagrammtyp**\
  Es ist möglich, eine Verknüpfung für jeden in Substance Designer verfügbaren Diagrammtyp zu definieren, von einem normalen Compositing-Diagramm bis hin zu fortgeschritteneren Funktionen und MDL.

  ![](../../../assets/shortcuts-graph-type.png)
* **Zuweisen von Tastaturbefehlen für atomare Knoten und benutzerdefinierte Filter**\
  Standardmäßig zeigt die Oberfläche die atomaren Knoten für jeden Diagrammtyp an. Mithilfe des Suchfelds können Sie die Liste verfeinern, aber auch benutzerdefinierte Bibliotheksfilter und Knoten zugänglich machen:

  ![](../../../assets/shortcut-node-search.gif)
* **Erstellen einer neuen Verknüpfung**\
  Um einen neuen Tastaturbefehl für einen bestimmten Knoten zu erstellen, klicken Sie einfach auf das Textfeld neben dem Namen des Knotens und geben die gewünschten Tasten auf der Tastatur ein. Um eine Verknüpfung zu löschen, verwenden Sie das Kreuz.
...
Während der SIGGRAPH 2019 haben wir ein MaterialX Plugin für Substance Designer vorgestellt. [MaterialX](http://www.materialx.org/) ist ein von [Industrial Light &amp; Magic (ILM)](https://www.ilm.com/) entwickeltes Format, mit dem Künstler Materialien erstellen können, die dann für eine Vielzahl von Plattformen und viele Verwendungszwecke (wie Filme, Fernsehsendungen, Spiele, VR-Erlebnisse und Fertigung) verwendet werden können. Dieses neue Plugin, das nun als Beta verfügbar ist, ermöglicht es, MaterialX-Materialien zu erstellen, die in anderen kompatiblen DCC-Anwendungen verwendet werden können. Werfen Sie einen Blick auf unseren letzten [Artikel über MaterialX](https://magazine.substance3d.com/research-a-portable-shader-graph/), um mehr über den Prozess und die Ziele dieses neuen Plug-ins zu erfahren.

Dieses neue Plug-in bietet folgende Möglichkeiten:

* Entwickle prozedurale Shader und Texturen parallel.
* Erstellen erweiterter Shader für Substance Painter mit Funktionen wie Detailzuordnungen und Prozedurmasken, die zur Laufzeit geändert werden können.
* Funktion eines Shaders, der eine Game-Engine oder eine VFX-Pipeline im Viewport für Substance Designer und Substance Painter verwendet.
...
Zusätzlich zu OpenColorIO ist es jetzt möglich, Adobe Color Engine (ACE) zur Unterstützung von ICC-Profilen für das Farbmanagement zu verwenden. Das bedeutet, dass Sie jetzt kalibrierte Bilder importieren und exportieren können, damit sie mit anderer Software wie Photoshop und Illustrator übereinstimmen.
...
* **ACES für Legacy-Farbmanagement**\
  Die tonemapped ACES-Ausgabetransformation ist jetzt mit dem Legacy-Farbmanagementmodus verfügbar.

### Verbesserte Bäcker

![](../../../assets/banner-baking-2.jpg)

Wir verfeinerten einige unserer Bäcker mit zwei wichtigsten Änderungen:

* **Verbessertes Sampling**\
  Der Bäcker für die Verdeckung, Thickness und Krümmung der Umgebung verwendet jetzt ein neues Sampling-Verfahren, das die Qualität der gebackenen Ergebnisse erheblich verbessert. Es erzeugt ein ansprechenderes Rauschen und stabilere Ergebnisse, ohne dass die Sampling-Anzahl erhöht werden muss. Die Abbildung unten zeigt die vorherige Sampling-Methode oben und die neue Methode unten mit 4, 8 und 16 Samples von links nach rechts (alle Renderings wurden mit Anti-Aliasing mit einer 4x4-Subsampling-Einstellung durchgeführt).

  ![](../../../assets/bakers-sampling-comparison.jpg){width="600px"}
* **Neue Normalisierungseinstellung**\
  Die Bäcker &quot;Thickness&quot; und &quot;Heightmap&quot; verfügen über einen neuen Normalisierungsparameter, mit dem Sie RAW-Werte in Texturen anstatt in einem Bereich von 0-1 festgeklemmter Werte backen können. Dies ist bei einer Height-Map nützlich, um die Rohabstände zwischen einem Low-Poly- und High-Poly-Gitter zu kennen. Der Divisor-Wert entspricht genau der Versatz-Intensität, die auf dem Low-Poly-Gitter eingerichtet werden soll, um der Silhouette des High-Poly-Gitters zu entsprechen. Zur korrekten Interpretation der Height-Map haben wir auch einen neuen Parameter &quot;Scalar Zero Value&quot; zur Definition des Pixelwertes 0 angezeigt. Darüber hinaus ermöglicht der manuelle Faktor, potenzielle Nahtprobleme bei UDIM-Netzen zu beheben, die durch eine Normalisierung per UV-Tile verursacht werden.
...
* **Textureingabevorschau anzeigen und Deaktivierungsparameter hinzufügen**\
  Shader-Eingabeparameter zeigen jetzt eine Vorschau ihrer Eingabe an: Es kann sich um einen einheitlichen Farbparameter, einen Diagrammknoten oder eine Bitmap handeln. Es ist auch möglich, einen Parameter zu deaktivieren, um ihn vorübergehend zu deaktivieren.

  ![](../../../assets/3dview-map-preview.png)

  ![](../../../assets/3dview-enable-setting.gif)
* **Vorschau der Umgebungszuordnung in den Einstellungen**\
  Wenn Sie zu **3D-Ansicht > Umgebung > Bearbeiten** wechseln, wird jetzt eine Vorschau der Umgebungszuordnung angezeigt, mit der die Szene beleuchtet wurde.

  ![](../../../assets/3dview-env-settings.jpg)
* **Neuer unbeleuchteter Shader**\
  Ein neuer Shader mit dem Namen &quot;unlit&quot; wurde standardmäßig hinzugefügt. Es reagiert nicht auf Beleuchtung und kann stattdessen verwendet werden, um eine einzelne Textur im Viewport über ein Gitter anzuzeigen. Das ist nützlich, um z. B. Texturen im Ofen zu betrachten.
